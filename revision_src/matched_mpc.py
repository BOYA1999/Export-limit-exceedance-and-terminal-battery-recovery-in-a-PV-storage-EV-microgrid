...
def matched_mpc_action(env, horizon=24, future_ramp_relaxed=False, export_priority=False):
    ...
    if export_priority:
        ...
        if phase1_feasible:
            ...
            phase2, feasible, violation = solve(objective, priority_matrix, np.r_[np.asarray(lo), -np.inf], np.r_[np.asarray(hi), export_limit])
            result = phase2
    ...
    info = {
        ...
        "phase1_mip_gap": float(phase1.mip_gap) if phase1 is not None and getattr(phase1, "mip_gap", None) is not None else None,
        "phase1_mip_dual_bound": float(phase1.mip_dual_bound) if phase1 is not None and getattr(phase1, "mip_dual_bound", None) is not None else None,
        "phase1_node_count": int(phase1.mip_node_count) if phase1 is not None and getattr(phase1, "mip_node_count", None) is not None else None,
        "phase1_constraint_violation_max": phase1_violation if phase1_feasible else None,
        "phase2_status": int(phase2.status) if phase2 is not None else None,
        "phase2_feasible": bool(phase2_feasible) if phase2_feasible is not None else None,
        "phase2_mip_gap": float(phase2.mip_gap) if phase2 is not None and getattr(phase2, "mip_gap", None) is not None else None,
        "phase2_mip_dual_bound": float(phase2.mip_dual_bound) if phase2 is not None and getattr(phase2, "mip_dual_bound", None) is not None else None,
        "phase2_node_count": int(phase2.mip_node_count) if phase2 is not None and getattr(phase2, "mip_node_count", None) is not None else None,
